...
{
    [Inject] public MusicFieldView view { get; set; }

    [Inject] public IGameStateData gameStateData { get; set; }

    [Inject] public MainGameContext mainGameContext { get; set; }
    ...
    {
        return (view.enhancedScroller.ScrollSize - gameStateData.musicLength * GetNodeCollumSize()) /
               view.enhancedScroller.ScrollSize;
    }
    ...
    {
        NodeCollumTileData nodeCollumData =
            new NodeCollumTileData(gameStateData.collumDatas.Count, gameStateData.tempo);
    ...
    {
        for (var i = cellDatas.Count - 1; i >= index; i--)
        {
    ...
    public void ContainerMoveUpdate(float deltaTime)
    {
        ...
        if (currentPos <= scrollCollumMoveAmount)
        {
            ...
            if (!gameStateData.isLoop && view.musicBar.transform.localPosition.x > 0)
            {
        ...
        else
        {
            ...
            view.musicBar.SetLocalPositionX(barScrollSize *
                                            ((currentPos - scrollCollumMoveAmount) / barCollumMoveAmount));
        }
    }
    ...
    public void RemoveEmptyCell(int minIndex = 0)
    {
        while (true)
        {
            ...
            var lastStaffNotes =
                cellDatas.GetRange(cellDatas.Count - gameStateData.musicLength, gameStateData.musicLength);
            if (!lastStaffNotes.Exists(e =>
                e is NodeCollumTileData && (e as NodeCollumTileData).emoDatas.Exists(emoData => emoData != null)))
            {
                RemoveNodeRange(cellDatas.Count - gameStateData.musicLength);
                view.enhancedScroller.ScrollRect.content.AddWidth(
                    -(gameStateData.musicLength * new NodeCollumTileData().GetCellViewSize()));
            }
            else
            {
                return;
