...
public class NodeCollumTileView : EnhancedScrollerCellView
{
    ...
    private void PopulateNodeSlot(NodeCollumTileData data)
    {
    ...
    private void SetCollumLine(int dataIndex, IGameStateData gameStateData)
    {
        if (dataIndex % gameStateData.musicLength == 0)
        {
            ...
            endTabLine.SetActive(false);
            inTabLine.SetActive(false);
        }
        else
        {
            ...
            {
                endTabLine.SetActive(true);
                inTabLine.SetActive(false);
            }
            else
            {
                endTabLine.SetActive(false);
                inTabLine.SetActive(true);
            }
        }
    }
    ...
    public void DoMoveNote(int startNoteIndex)
    {
        for (int i = 1; i < noteViews.Count; i++)
        {
            ...
            if (noteViews[i - 1].positionType != noteViews[i].positionType &&
                noteViews[i - 1].positionType != NotePositionType.Center &&
                noteViews[i].positionType != NotePositionType.Center)
            {
                continue;
            }
